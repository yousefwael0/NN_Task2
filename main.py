...
with st.sidebar:
    ...
    for i in range(int(n_hidden)):
        n = st.number_input(
            ...
            min_value=1, max_value=64, value=4, step=1, key=f"neurons_{i}"
        )
        neurons_per_layer.append(int(n))

    eta = st.number_input(
        "Learning rate (η)", min_value=0.0001, max_value=1.0,
    ...
    activation = st.radio(
        ...
        format_func=lambda x: "Sigmoid" if x == "sigmoid" else "Hyperbolic Tangent (tanh)"
    )
...
if acc_log:
    st.line_chart(
        pd.DataFrame({"Training Accuracy (%)": acc_log}),
...
st.dataframe(
    cm_df.style.highlight_max(axis=None, color="#d4edda"),
    use_container_width=True
)
